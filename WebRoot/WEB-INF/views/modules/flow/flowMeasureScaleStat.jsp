...
<body>
	<ul class="nav nav-tabs">
		...
		<li  class="active"><a href="${ctx}/flow/flowMeasureScale/analysis">流量观测尺度数据分析</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="flowMeasureScale" action="${ctx}/flow/flowMeasureScale/analysis" method="post" class="breadcrumb form-search">
		
		<ul class="ul-form" style="min-height:100px;">
			
			<li><label>统计月份：</label>
				<li><label>发生日期：</label>
				<input id="beginFindTime" name="beginFindTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${flowMeasureScale.beginFindTime}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> -
				<input id="endFindTime" name="endFindTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${flowMeasureScale.endFindTime}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
				</li>
			</li>
			<li><label>统计类别：</label>
			<form:select path="flowType" class="input-xxlarge " multiple="multiple">
					<form:options items="${fns:getDictList('flow_scale')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>断面筛选：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered  table-condensed">
		...
		<tbody>
		<c:forEach items="${flowMeasureScaleList}" var="flowMeasures">
			<tr>
